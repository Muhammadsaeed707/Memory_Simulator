                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  int offset = (address & WORD_OFFSET_MASK) >> WORD_OFFSET_SHIFT;
  ...
  *status = 0;
  for(int i = 0; i < L1_LINES_PER_SET; i++){
      if((l1_cache[index].lines[i].v_r_d_tag & L1_VBIT_MASK) != 0 && (l1_cache[index].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK) == tag) {
        l1_cache[index].lines[i].v_r_d_tag = l1_cache[index].lines[i].v_r_d_tag | L1_RBIT_MASK;
        *status = 1;
        if((control & 0x1) != 0) {
          *read_data = l1_cache[index].lines[i].cache_line[offset];
        }
        if((control & 0x2) != 0) {
          l1_cache[index].lines[i].cache_line[offset] = write_data;
                    ...
                    uint8_t *status)
{
  ...
  // can return
    ...
    if ((l1_cache[index].lines[i].v_r_d_tag & L1_VBIT_MASK) == 0) {
      for (int j = 0; j < 8; j++) {
  ...
  //  the first entry that has r=0 and d=1, etc. 
    else if (((l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) == 0) && ((l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) == 0) && (r0_d0_index == ~0x0)) {
      r0_d0_index = i;
    }
    else if (((l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) == 0) && ((l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) != 0) && (r0_d1_index == ~0x0)) {
      r0_d1_index = i;
    }
    else if (((l1_cache[index].lines[i].v_r_d_tag & L1_RBIT_MASK) != 0) && ((l1_cache[index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) == 0) && (r1_d0_index == ~0x0)) {
      r1_d0_index = i;
    }
  }
  ...
  if ((r0_d0_index) != ~0x0) {
    set_index = r0_d0_index;
  }
  else if ((r0_d1_index) != ~0x0) {
    set_index = r0_d1_index;
  }
  else if ((r1_d0_index) != ~0x0) {
    set_index = r1_d0_index;
  }
  ...
  *evicted_writeback_address = ((l1_cache[index].lines[set_index].v_r_d_tag & L1_ENTRY_TAG_MASK) << L1_ADDRESS_TAG_SHIFT) | (index << L1_SET_INDEX_SHIFT);
  if ((l1_cache[index].lines[set_index].v_r_d_tag & L1_DIRTYBIT_MASK) != 0) {
    for(int i = 0; i < 8; i++) {
      evicted_writeback_data[i]=l1_cache[index].lines[set_index].cache_line[i];
    }
    *status = 1;
  }
  ...
  for (int j = 0; j < 8; j++) {
    l1_cache[index].lines[set_index].cache_line[j] = write_data[j];
  }
  l1_cache[index].lines[set_index].v_r_d_tag = l1_cache[index].lines[set_index].v_r_d_tag | L1_VBIT_MASK;
  l1_cache[index].lines[set_index].v_r_d_tag = l1_cache[index].lines[set_index].v_r_d_tag & (~L1_DIRTYBIT_MASK);
  l1_cache[index].lines[set_index].v_r_d_tag = (l1_cache[index].lines[set_index].v_r_d_tag & (~L1_ENTRY_TAG_MASK)) | tag;
}
