...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  uint32_t read[WORDS_PER_CACHE_LINE];
  uint8_t status;
  l2_cache_access(address, 0,READ_ENABLE_MASK ,read,&status);
  ...
  uint32_t evicted_writedata[WORDS_PER_CACHE_LINE];
  l1_insert_line(address, read, &evicted_writeback_address, evicted_writedata, &status);
  ...
  if ((status & 0x1)) {
    l2_cache_access(evicted_writeback_address,evicted_writedata,WRITE_ENABLE_MASK,0,&status);
    if (!(status & 0x1)) {
      memory_handle_l2_miss(evicted_writeback_address,WRITE_ENABLE_MASK);
      l2_cache_access(evicted_writeback_address, read, WRITE_ENABLE_MASK, 0, &status);
    }
  }
}
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  if (control & READ_ENABLE_MASK) {
    main_memory_access(address, 0, READ_ENABLE_MASK, cache_line);
  }
  ...
  uint32_t evicted_writedata[WORDS_PER_CACHE_LINE];
  uint8_t status;
  l2_insert_line(address, cache_line, &evicted_writeback_address, evicted_writedata, &status);
  ...
  if ((status & 0x1)) {
    main_memory_access(evicted_writeback_address, evicted_writedata, WRITE_ENABLE_MASK, 0);
  }
}
