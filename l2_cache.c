                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  int address_index = (address & L2_INDEX_MASK) >> L2_INDEX_SHIFT;
  ...
  int address_tag = (address & L2_ADDRESS_TAG_MASK) >> L2_ADDRESS_TAG_SHIFT; 
  ...
  if ((l2_cache[address_index].v_d_tag & L2_VBIT_MASK) == 0 || (l2_cache[address_index].v_d_tag & L2_ENTRY_TAG_MASK) != address_tag) {
    *status = 0;
  }
  ...
  else if ((control & READ_ENABLE_MASK) != 0) {
    for (int i = 0; i < 8; i++){
      read_data[i] = l2_cache[address_index].cache_line[i];
    }

    *status = 1;
  }

  else if ((control & WRITE_ENABLE_MASK) != 0) {
    for (int i = 0; i < 8; i++) {
      l2_cache[address_index].cache_line[i] = write_data[i];
    }

    l2_cache[address_index].v_d_tag = l2_cache[address_index].v_d_tag | L2_DIRTYBIT_MASK;
    *status = 1;
  }
}
                    ...
                    uint8_t *status)
{
  ...
  if ((l2_cache[address_index].v_d_tag & L2_VBIT_MASK) == 0 || (l2_cache[address_index].v_d_tag & L2_DIRTYBIT_MASK) == 0) {
    for (int i = 0; i < 8; i++) {
      l2_cache[address_index].cache_line[i] = write_data[i];
    }
    l2_cache[address_index].v_d_tag = l2_cache[address_index].v_d_tag | L2_VBIT_MASK;
    l2_cache[address_index].v_d_tag = l2_cache[address_index].v_d_tag & (~L2_DIRTYBIT_MASK);
    l2_cache[address_index].v_d_tag = l2_cache[address_index].v_d_tag & (~L2_ENTRY_TAG_MASK);
    l2_cache[address_index].v_d_tag = l2_cache[address_index].v_d_tag | address_tag;
    *status = 0;
    return;
  }
  ...
  //the write-back is needed.
    ...
    *evicted_writeback_address = ((l2_cache[address_index].v_d_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) | (address_index << L2_INDEX_SHIFT);
    for(int i = 0; i < 8; i++) {
      evicted_writeback_data[i]=l2_cache[address_index].cache_line[i];
    }
    *status = 1;
  }
